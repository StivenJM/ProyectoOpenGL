...
public:
    ...
    GameObject(glm::vec3 position, glm::vec3 size) : Position(position), Size(size) {}
};
...
// La posicion de todos los cubos (por defecto tienen un dimensiones de x, y, z de 1.0f)
glm::vec3 cubes[] = {
...
int main()
{
    ...
    // ------------------------------------
    Shader ourShader("shaders/tests/shader_exercise12.vs", "shaders/tests/shader_exercise12.fs");
    ...
    unsigned char* data = stbi_load("textures/tests/container.jpg", &width, &height, &nrChannels, 0);    if (data)
    if (data)
    {
    ...
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
    // load image, create texture and generate mipmaps
    data = stbi_load("textures/tests/awesomeface.png", &width, &height, &nrChannels, 0);    if (data)
    if (data)
    {
    ...
    while (!glfwWindowShouldClose(window))
    {
       ...
       // --------------------
        ...
        // Se cambia la posicion del AABB de la camara
...
void BlockMovement(GameObject& player, GameObject& obstacle)
{
    ...
    if (player.Position.z > obstacle.Position.z)
        block_backward = true;
    else if (player.Position.z < obstacle.Position.z)
        block_fordward = true;
}

void DoCollisions()
{
    for (glm::vec3 p : cubes)
    {
        p -= glm::vec3(-0.5f, -0.5f, -0.5f);
        GameObject op = GameObject(p, glm::vec3(1.0f));

        GameObject jugadorAuxiliar = jugador;
        jugadorAuxiliar.Position -= glm::vec3(-0.5f, -0.5f, -0.5f);
        if (CheckCollision(jugadorAuxiliar, op))
        {
            BlockMovement(jugadorAuxiliar, op);
        }
    }
}